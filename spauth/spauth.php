...
//		$query = "SELECT * FROM spauth WHERE license_code = :license_code";
		...
		if($row){
			...
			//파일경로 가져오기
			$icon_url = "http://".str_replace("//","/","license.starplayer.net/license/files/".$row['service_icon']);
			$image_url = "http://".str_replace("//","/","license.starplayer.net/license/files/".$row['launcher_image']);
