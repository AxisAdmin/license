...
//				exit;
				if($val['error'] != 4){	//파일이 없지 않으면
					...
					for ($i = 5000; $i < $config['max_filename_increment']; $i++)
					{
						if (!in_array($gConf['ftp_path'].$key.$i.$extension, $contents))
						{
							$new_filename = $gConf['ftp_path'].$key.$i.$extension;
							break;
						}
					}

					/* ftp_put($conn_id, 원격서버에 업로드될 파일명, 현재 서버의 파일명, 모드) */
					$upload = ftp_put($conn_id, $new_filename, $_FILES[$key]["tmp_name"], FTP_BINARY);
